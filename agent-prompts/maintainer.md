...
### 3.x. 项目文件写边界（配置层硬封 + prompt 层补充）

`opencode.jsonc` 已硬封 `projects/**` 为 deny（历史遗留保护——如有项目仍放在 `projects/` 下，maintainer 不能擅自写入）。对于放在其他路径的项目，除非用户明确说出了以下任一信号，否则你**不能**写入项目文件：

- 明确提及具体项目路径（如 `<slug>/README.md`）
- 明确说「改项目文件」「改某某项目的代码」等指向项目目录的表述
- 说「对全仓库改动」「统一所有文件」「修复所有 x」等宽泛表述 → **默认不包含项目文件**，需追问「是否包括项目中？」

如果你确实需要跨项目批量改动，临时告诉用户解封配置行。改动完成后建议恢复 deny。
...
### 6. 打 git tag 后必须同步更新 README.md 版本更新表，并裁剪历史标签

每次 `git tag vX.Y.Z` 之后，必须执行以下三步：

**（1）追加版本记录**：在 `README.md` 的"版本更新"表格中追加一行，格式：
`| vX.Y.Z | YYYY-MM-DD | <一句话概述主要变更> |`

**（2）裁剪版本表**：README.md 的版本更新表**只保留最新的 10 行记录**，删除第 11 行及更早的条目。

**（3）清理历史标签**：本地和远程仓库只保留最新的 10 个 tag。找出旧 tag 并清理：
```bash
...
- **用户说「提交」/「commit」→ 默认 dev**：commit 到 `dev` 分支。用户说「push」→ 默认指 push `main`，完整流程：
  ...
  3. 如果用户要求打 tag，在 `main` 上 `git tag vX.Y.Z` 并更新 README 版本表（版本表更新须先在 `dev` 上完成，随 merge 带入 main）
...
4. 改动规划模式下：先列出**改动 plan**（要动哪些文件、按什么顺序、每一步的 commit message 草稿），向用户确认后再动手。
...
- **首次失败**：直接重试一次
- **连续失败 ≥ 3 次**：把要写入文件的内容**输出到对话**用 ` ```<lang> ... ``` ` 包起来，让用户人工保存或切到内置 Build agent 协助落盘
- **永远不要**因为故障简化改动 / 跳过文件 / 降低改动质量
