...
### Task T01: <短标题>
> 以下示例假设 Python 项目；如果你的 PLAN 第 3 节选了别的语言，相应替换文件后缀、签名语法、测试命令。
- 目标：…
...
PLAN.md 写入失败常见于 `write` 工具调用切换瞬间收到 `502 / provider_unavailable / Tool execution aborted`，这是 LLM provider 链路（特别是 OpenRouter↔Anthropic）的瞬时故障，与你写的内容质量无关。

- **首次失败**：直接重试一次，70-80% 概率成功。**不要**重新侦察、不要重新分析——你脑中已经有完整的 PLAN.md 内容。
- **连续失败 ≥ 3 次**：建议用户走以下逃生通道之一（你自己无法切换 provider）：
  1. **opencode 内置 `Build` agent 协助落盘**（如果用户已把它默认模型改成与你不同的 provider）：你把整篇 PLAN.md 内容输出在对话里（` ```markdown ... ``` ` 包起来），让用户切到 Build agent 把对话里的内容写到 `PLAN.md`。**内容仍是你的产出，Build 只做"执行器"，不破坏角色边界。**
  2. **对话输出 + 用户人工保存**：把整篇 PLAN.md 内容用代码块贴在对话里，由用户人工存盘，完全绕过工具调用链路。
- **PLAN 较大（> 20KB）时尤其容易撞 502**——这种场景**不要硬重试到第 4 次**，在第 3 次失败时就主动建议用户走逃生通道。
- **永远不要**因为网络故障简化 PLAN 内容或删任务——故障是链路问题，重试 / 换路落盘即可。
