...
**PDF 处理优先级**：处理 PDF 文件时，agent 应遵循「先提取、后 OCR」的顺序，避免对 born-digital PDF 做无意义的 GPU 推理：

1. **先调 `pdf_to_text`**（PyMuPDF，毫秒级）：提取 PDF 中已嵌入的文本。若返回充足文本，任务完成。
2. **文本为空或不完整时，再调 `ocr_glm`**（GLM-OCR 0.9B VLM，~20 秒/页）：对扫描件、图片型 PDF 做视觉 OCR。`ocr_glm` 已内置异步任务队列，多页 PDF 不会因超时失败。
